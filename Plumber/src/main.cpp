...
bool hasInput = false, isFinish = false;// Define boolean variables to track input status and game completion status
...
int main() {
	...
	do
	{
		if (isVaild(strHeight) == true && isVaild(strWidth) == true) {
	...
	/*start = randomPos();
	mEnd = randomPos();
	while (start.x == mEnd.x && start.y == mEnd.y) {
		mEnd = randomPos();
	}*/

	cout << start.x << " " << start.y << " " << mEnd.x << " " << mEnd.y << "\n";
...
void drawBoard() {
	...
	for (size_t i = 0; i < height * 3; i++) //output board
	{
		for (size_t j = 0; j < width * 4; j++)
		{
			...
			{
				SetConsoleTextAttribute(hConsole, 154); //make the color of console into blue words
			}
...
void keyUpdate() {
	...
	switch (input) {
		//w,a,s,d are for selecting targets
	case 'w'://move up
